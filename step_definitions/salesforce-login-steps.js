...
Then(/^The user accesses the form to create a new account$/, async () => {
  await helpers.loadPage(
    `${env.salesforceInstanceUrl}/lightning/o/Account/list?filterName=Recent`,
    10
  );
});

Then(/^They create a new client account$/, async () => {
  const newAccountBtn = await browser.$(
    sharedObjects.salesforceData.elem.newAccountBtn
  );
  await newAccountBtn.waitForDisplayed({ timeout: DELAY_10s });
  await newAccountBtn.click();

  const accountFormModal = await browser.$(
    sharedObjects.salesforceData.elem.accountFormModal
  );
