...
module.exports = {
  getAuthToken: async () => {
    ...
    await elem.waitForExist({ timeout: DELAY_10s });
  ...
  getCall: async () => {
    for (links of apiGetEndPoints) {
      ...
      await helpers.apiCall(url, 'GET', auth, null);
    }
  },
  ...
  postCall: async () => {
    ...
    for (links of apiPostEndPoints) {
      ...
      fs.writeFileSync(filePath, JSON.stringify(comboFile, null, '\t'));
  ...
  deleteCall: async () => {
    ...
    for (links of apiDeleteEndPoints) {
      ...
      console.log('delete resp value: ', resp.body);
    }
  },
};
